...
<?php
	...
	echo "</head>";
		...
		echo "<h2>Cat&eacute;gories priv&eacute;es<h2>";

		echo "<div align = \"center\">";
			echo "<table class = \"menu-boutons\">";
				echo "<tr>";
					echo "<td>";
						echo "<A HREF = \"favoris_publics.php\" target = \"body\"><img height=\"$hauteur_icone_action\" width=\"$largeur_icone_action\" src = \"$chemin_theme_images/favoris_categorie.png\" ALT = \"favoris publics\" border = \"0\" title=\"Favoris publics\"></a>
							<br /><span class=\"IconesAvecTexte\">Favoris publics</span><br />";
					echo "</td>";
					/*
					echo "<td>";
						echo "<img height=\"$hauteur_icone_action\" width=\"$largeur_icone_action\" src = \"$chemin_theme_images/favoris_categorie_privee.png\" ALT = \"favoris priv&eacute;s\" border = \"0\" title=\"Favoris priv&eacute;s\"></A>
		...
		$test_categ_prive = "SELECT id_categ, intitule_categ FROM favoris_categories WHERE id_util = $id_util ORDER BY intitule_categ";
		$resultat_categ_prive = mysql_query($test_categ_prive);

		echo "<table id = \"tablefavoris\">";
		$cat=1; //Permet d'identifier chaque tableau par catégorie
		while ($categorie = mysql_fetch_row($resultat_categ_prive)) // on extrait ligne par ligne due la table catégories
		{
						...
						echo "<table id=\"tablefavoris\">";
							...
							echo "<tr align = \"center\">";
								echo "<td>";
									...
									$requete_favoris_prive = "SELECT id_favori, intitule, adresse FROM favoris WHERE id_categ = $categorie[0] order by intitule";
									$resultat_requete_favoris_prive = mysql_query($requete_favoris_prive);
									$nb_favoris = mysql_num_rows($resultat_requete_favoris_prive);
									if ($nb_favoris == 0)
									{
									...
									{
										while ($favori = mysql_fetch_row ($resultat_requete_favoris_prive)) // Tant qu'il y a des favoris pour la catégorie en cours
										{
											echo "<tr>

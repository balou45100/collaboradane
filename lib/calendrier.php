...
// mettre cette variable à true si les dates antérieurs à la date d'aujourd'hui peuvent etre cliquable
$date_anterieur_cliquable = true ;
...
//**************************************************************************************************
 ...
 $resultat = false ;
 if ( $format_date == "eng" ) {
    $temp = explode ( $separateur, $date_debut);
    $jour_debut = $temp[2];
    $mois_debut = $temp[1];
    $annee_debut = $temp[0];
    $temp = explode ( $separateur, $date_fin);
    $jour_fin = $temp[2];
    $mois_fin = $temp[1];
    $annee_fin = $temp[0];
 }
              ...
              if ( $premier_jour_depasse && ($compteur_jour <= $numero_dernier_jour_mois) && $j < 8)
                  {
                    ...
                    $ctrl_date_cliquable = ( $date_anterieur_cliquable || comparaison_date ($compteur_jour.$format_separateur_date.$mois_en_cours.$format_separateur_date.$annee_en_cours,date("d/m/Y"),"inferieur_egale",$format_separateur_date,"fr") ) ? true : false ;
                    if ( $date_lien == 0 && $format_date_fr && $ctrl_date_cliquable ) {
                        echo '<a href="javascript:window.opener.document.getElementById(\'',$nom_champs_selecteur,'\').value=\'',ajout_supprime_zero ($compteur_jour.$format_separateur_date.$mois_en_cours.$format_separateur_date.$annee_en_cours,"Ajout",$format_separateur_date,"fr"),'\';;window.close();" class = date',$class_date_lien,' >';
                        }
                    if ( $date_lien == 0 && (!($format_date_fr)) && $ctrl_date_cliquable  ) {
                        echo '<a href="javascript:window.opener.document.getElementById(\'',$nom_champs_selecteur,'\').value=\'',ajout_supprime_zero ($annee_en_cours.$format_separateur_date.$mois_en_cours.$format_separateur_date.$compteur_jour,"Ajout",$format_separateur_date,"eng"),'\';;window.close();" class = date',$class_date_lien,' >';
                       }

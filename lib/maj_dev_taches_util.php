...
<?php
	...
	//on balaie la table dev_taches pour extraire les créateurs et de les balancer dans la table dev_taches_util
	/*
	$util = array ();
	$i = 0;
	$j = 0;
	*/
	$taches="SELECT id_tache, id_util_creation, id_util_traitant FROM dev_taches ORDER BY id_tache";
	...
	while ($results = mysql_fetch_row ($query_taches))
	{
		...
		//On vérifie si la tâche existe dans la table dev_taches_util
		$verif_tache="SELECT id_tache, id_util, statut_cta FROM dev_taches_util WHERE id_tache = $results[0]";
		...
		else
		{
			...
			if ($id_util_creation == $id_util_traitant)
			{
				...
				$requete_ajout_fiche_tache = "INSERT INTO `dev_taches_util` (`id_tache`,`id_util`,`statut_cta`)
			...
			/*
			$requete_ajout_fiche_tache = "INSERT INTO `dev_taches_util` (`id_tache`,`id_util`,`statut`)
		...
		//On vérifie si le créateur existe dans la table dev_taches_util
		$verif_crea="SELECT id_tache, id_util, statut_cta FROM dev_taches_util WHERE id_tache = $results[0] AND id_util = $results[1] AND (statut_cta = '100' OR statut_cta = '110')";
		...
		//On vérifie si le traitant existe dans la table dev_taches_util
		$verif_trait="SELECT id_tache, id_util, statut_cta FROM dev_taches_util WHERE id_tache = $results[0] AND id_util = $results[2] AND (statut_cta = '110' OR statut_cta = '010')";
		...
		else
		{
			...
			/*
			$requete_ajout_fiche_traitant = "INSERT INTO `dev_taches_util` (`id_tache`,`id_util`,`statut`)

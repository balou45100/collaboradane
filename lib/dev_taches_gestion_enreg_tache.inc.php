...
//////////////////////////////////////////////////////////////////////////
	...
	$requete_ajout = "INSERT INTO `dev_taches` (`date_creation`,`date_echeance`,`date_rappel`,`description`,`etat`,`visibilite`,`priorite`,`observation`)
...
*/
	...
	//On renseigne la table dev_taches_util avec le créateur et la personne traitant la tâche
	//Il faut regarder si le créateur et le traitant sont la même personne
	if (($id_util == $id_util_traitant) OR ($id_util_traitant == 0))
	{
		$requete_ajout_tache_util = "INSERT INTO `dev_taches_util` (`id_tache`,`id_util`,`statut_cta`)
...
*/
		$requete_ajout_tache_traitant = "INSERT INTO `dev_taches_util` (`id_tache`,`id_util`,`statut_cta`)
...
*/
	...
	//On renseigne la table dev_taches_util avec les personnes associées
	...
	if (($id_util_associes[0] <> 0) OR ($id_util_associes[0] == 0 AND $nombre_elements >1))
	{
		...
		while($i < $nombre_elements)
		{
			...
			if (($id_util_associes[$i] <> $id_util) AND ($id_util_associes[$i] <> $id_util_traitant) AND $id_util_associes[$i] <> 0)
			{
				...
				$requete_ajout_personne_associee = "INSERT INTO `dev_taches_util` (`id_tache`,`id_util`,`statut_cta`)
...
*/	
	//On renseigne la table dev_taches_modules si nécessaire
/*
	if ($id_module <> "")
	{
		$requete_enreg_categ = "INSERT INTO dev_taches_modules (`id_tache`,`id_module`)
			VALUES ('".$id_tache_insere."','".$id_module."')";
	...
	if (($id_module[0] <> 0) OR ($id_module[0] == 0 AND $nombre_elements >1))
	{
		...
		while($i < $nombre_elements)
		{
			...
			if ($id_module[$i] <> 0)
			{
				//echo " - On enregistrera la fiche";
				$requete_enreg_categ = "INSERT INTO dev_taches_modules (`id_tache`,`id_module`)
					VALUES ('".$id_tache_insere."','".$id_module[$i]."')";
				$resultat_enreg_categ = mysql_query($requete_enreg_categ);
				if (!$resultat_enreg_categ)
				{
					echo "<br>Erreur lors de l'enregistrement de la cat&eacute;gorie";

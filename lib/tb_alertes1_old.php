...
<?php
		...
		$query1="SELECT id_alerte, id_ticket, description, NOM, TEXTE, date_alerte FROM alertes, probleme WHERE alertes.id_ticket = probleme.ID_PB and id_util = $id order by date_alerte";
		...
		if($num_results1 >= "1")
		{
			...
			if($num_results1 >= "1")
			{
				...
				if($num_results1 == 1)
				{
					echo "<h3>Vous avez $num_results1 alerte &nbsp &nbsp</h3>";
				}
				else
				{
					echo "<h3>Vous avez $num_results1 alertes &nbsp &nbsp</h3>";
				}
				// On affiche le tableau...	
				echo "<table border = 1 bgcolor=#48D1CC><tr><td><center>Date alerte</center></td><td><center>Nom</center></td><td><center>Description</center></td><td><center>Infos</center></td></tr>";
				// MEME SCRIPT QUE LIGNE 51
				while($results1 = mysql_fetch_row($execution1))
				{
					$date_auj = time();
					list($annee, $mois, $jour)= explode ("-",$results1[5]);
			...
			{
				echo "<h3>Vous n'avez aucune alerte &nbsp &nbsp</h3>"; 	
			}
		}
		else
		{

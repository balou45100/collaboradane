...
<?php
	...
	echo "</head>";
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			echo "<form action = \"documents_accueil.php\" target = \"body\" method = \"get\">";
				echo "<table class = \"menu-boutons\">";
					echo "<tr>";
						echo "<td>";
							...
							$requete_utilisateurs="SELECT DISTINCT u.ID_UTIL, u.NOM, u.PRENOM FROM util AS u, util_groupes AS ug, documents AS d
								WHERE u.ID_UTIL = ug.ID_UTIL
									AND ug.ID_GROUPE = 34
									AND d.id_util_deposant = u.ID_UTIL
									AND u.visible = 'O'
								ORDER BY NOM";
					...
					echo "<td>";
						$requete_modules="SELECT DISTINCT module FROM documents
							WHERE module <> 'COU'
								ORDER BY module";

								//echo "$requete_modules";

						$resultat_requete_module=mysql_query($requete_modules);
						$num_rows = mysql_num_rows($resultat_requete_module);
						echo "&nbsp;Module&nbsp;:&nbsp;<select size=\"1\" name=\"module_filtre\">";
						if (mysql_num_rows($resultat_requete_module))
						{
							echo "<option selected value=\"%\">tous</option>";
							while ($ligne=mysql_fetch_object($resultat_requete_module))
							{
								$module=$ligne->module;
								...
								echo "<option value=\"$module\">$intitule_module</option>";
							}
						}
						echo "</select>";
					echo "</td>";
					...
					echo "<td>";
						$requete_dossier="SELECT DISTINCT cc.id_categ,cc.intitule_categ FROM categorie_commune AS cc, documents_categories_communes AS dcc
							WHERE cc.id_categ = dcc.id_categorie_commune

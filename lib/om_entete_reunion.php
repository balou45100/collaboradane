...
<?php
	...
	echo "&nbsp;&nbsp;<form action=\"om_affichage_reunion.php\" target=\"body\" method=\"post\">";
			...
			echo "Intitul&eacute; r&eacute;union&nbsp;:&nbsp;<select size=\"1\" name=\"intitule_reunion\">";
			...
			echo "&nbsp;&nbsp;Etat :
				<select name=\"etat\">
					...
					<option value=\"0\">non trait&eacute;</option>
			...
			echo "&nbsp;&nbsp;Ann&eacute;e :
				...
				for ($i = $annee_en_cours; $i >= 2000; $i--)
				{
					//echo $i;
					$annee2 = $i+1;
					//$annee_a_afficher = $i."-".$i+1;
					
					echo "<option value=\"$i\">$i-$annee2</option>";
				}
			echo "</select>&nbsp;&nbsp;&nbsp";
	...
	//echo "</form>";
			...
			//$requete_date_reunion="SELECT DISTINCT date_horaire_debut FROM om_reunion ORDER BY date_horaire_debut DESC";
			$requete_date_reunion="SELECT DISTINCT DATE_FORMAT(date_horaire_debut, '%Y-%m-%d') AS Date_D FROM om_reunion ORDER BY Date_D DESC";
			
			
			//echo $requete_date_reunion;
			
			$result_date_reunion=mysql_query($requete_date_reunion);
			$num_rows = mysql_num_rows($result_date_reunion);
			echo "Date&nbsp;:&nbsp;<select size=\"1\" name=\"date_reunion\">";
			if (mysql_num_rows($result_date_reunion))
			{
		  
				echo "<option selected value=\"%\">Tous</option>";
				while ($ligne=mysql_fetch_object($result_date_reunion))
				{
					//$date_reunion=$ligne->date_horaire_debut;
					$date_reunion=$ligne->Date_D;
					echo "<option value=\"$date_reunion\">$date_reunion</option>";
				}
			}
			echo "</select>"; 
	...
	/*
		//echo "**************************************&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
		//echo "<form method=\"post\" target=\"body\" action=\"om_affichage_reunion.php\">";
		//echo "&nbsp;&nbsp;Date (jj-mm-aaaa)&nbsp;:&nbsp;<input type=\"text\" maxlength=\"10\" name=\"date\" id=\"dateDebut\" onfocus=\"visuCal(this);\" onblur=\"masqueCal(this);\" onkeyup=\"this.style.color=testTypeDate(this.value)\">&nbsp;&nbsp;&nbsp;";
		echo "&nbsp;&nbsp;Date (jj-mm-aaaa)&nbsp;:&nbsp;<input type=\"text\" maxlength=\"10\" name=\"date\" id=\"dateDebut\" >";
		//echo "<input type=\"submit\" name=\"validDate\" value=\"Recherche sur la date\" />";
	*/
		echo "<input type=\"submit\" name=\"validRecherche\" value=\"Rechercher\" />";
	echo "</form>";

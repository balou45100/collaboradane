...
@mysql_select_db("collaboratice"); // Notre base de données
...
$id=explode("/",$ids."////");
$chp=array("ville","no_societe","societe"); // Les noms des champs
...
// identifiant, libellé où identifiant est numérique auto-incrémenté
   case "1": // Liste des produits (ex : que des stylos )  
      $query = "SELECT no_societe, societe FROM repertoire ".$w." ORDER BY societe";
...
//  Si q n'est ni 1, ni 2, ni 3, il faut préparer le 0 ;)
   ...
   $liste_dpt = "";
   $SQL = "SELECT no_societe, ville FROM etablissements ORDER BY ville";
   $res = mysql_query($SQL);
